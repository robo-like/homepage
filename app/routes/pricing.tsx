...
export function meta({}: Route.MetaArgs) {
  return [
    { title: "RoboLike Pricing - Automate Your Social Media Growth" },
    {
      name: "description",
      content:
        "Choose the perfect RoboLike plan for your social media automation needs. From Free trial to Premium features.",
    },
  ];
}
...
function PricingTier({
  ...
  return (
    <Card
      className={`flex flex-col ${highlighted ? "border-2 border-lightPurple relative" : ""}`}
    >
      {highlighted && (
      ...
      <ul className="space-y-3 mb-8 flex-grow">
        {features.map((feature, index) => (
          <li key={index} className="flex items-center gap-2">
            <svg
              className="w-5 h-5 text-lightPurple"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
